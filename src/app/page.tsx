  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden">
        <div
          aria-hidden="true"
          className="pointer-events-none absolute -right-40 -top-40 h-[32rem] w-[32rem] rounded-full bg-brand-100/70 blur-3xl"
        />
        <div
          aria-hidden="true"
          className="pointer-events-none absolute -left-32 top-40 h-96 w-96 rounded-full bg-sand-100/80 blur-3xl"
        />
        <div className="container-page relative grid gap-14 py-16 sm:py-24 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            <p className="inline-flex items-center gap-2 rounded-full border border-brand-200 bg-brand-50 px-3.5 py-1.5 text-xs font-medium text-brand-800">
              <span className="h-1.5 w-1.5 rounded-full bg-brand-500" />
              Board-certified dermatologists · Bengaluru
            </p>
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              <Link
                href="/contact"
                className="rounded-full bg-brand-700 px-7 py-3.5 text-center text-sm font-semibold text-white transition-colors hover:bg-brand-800"
              >
                Book a consultation
              </Link>
              <Link
                href="/services"
                className="rounded-full border border-brand-200 bg-surface px-7 py-3.5 text-center text-sm font-semibold text-brand-800 transition-colors hover:border-brand-400"
              >
                Explore treatments
              </Link>
            </div>
              ...
              <ol className="mt-6 space-y-6">
                {[
                  ...
                  <li key={item.t} className="flex gap-4">
                    <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-brand-700 text-xs font-semibold text-white">
                      {i + 1}
                    </span>
                    <div>
      ...
      <Section className="bg-surface">
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-3">
          {testimonials.map((t) => (
            ...
            >
              <div className="flex gap-0.5 text-sand-400" aria-label="5 out of 5">
